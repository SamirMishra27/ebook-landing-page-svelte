<script>
	...
	function onFaqItemClicked(index) {
		const alreadyExpanded = questionCurrentlyExpanded === index;
		questionCurrentlyExpanded = alreadyExpanded ? -1 : index;
	}
</script>
	...
	<div class="faq-container">
		{#each faqs as faq, index (index)}
			<FaqItem
				...
				onclick={() => onFaqItemClicked(index)}
			/>
		{/each}
	</div>
